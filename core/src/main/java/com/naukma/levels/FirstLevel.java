...
public class FirstLevel extends BasicLevel {
    ...
    protected void initializeLevel() {
        availableFish.clear();
        timeLimit = 60f; // Збільшуємо час оскільки треба з'їсти більше риб
    ...
    protected int getFishUnlockRequirement(int fishTypeIndex) {
        ...
        switch (fishTypeIndex) {
            case 0:
                return 5; // Після 20 перших риб (fish_04/) розблоковується другий тип (fish_05/)
            case 1:
                return 2;  // Після 3 других риб (fish_05/) розблоковується третій тип (fish_02/)
            case 2:
                return 2;  // Після 2 третіх риб (fish_02/) - перемога
            default:
                return 5;
        }
    }
